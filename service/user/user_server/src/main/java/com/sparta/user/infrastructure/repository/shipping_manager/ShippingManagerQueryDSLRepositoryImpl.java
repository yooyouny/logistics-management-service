...
public class ShippingManagerQueryDSLRepositoryImpl implements ShippingManagerQueryDSLRepository {
  ...
  public Page<SippingManagerInfo> findShippingManagerInfos(UUID shippingManagerId, String keyword,
    ...
    JPAQuery<ShippingManager> query = queryFactory.select(shippingManager)
        .from(shippingManager);

    // shippingManagerId가 존재하는 경우 == ROLE_HUB_MANAGER
    if (shippingManagerId != null) {
      UUID hubId = queryFactory.select(shippingManager.hubId)
          .from(shippingManager)
    ...
    if (keyword != null && !keyword.isEmpty()) {
      query.where(shippingManager.slackId.containsIgnoreCase(keyword)
          .or(shippingManager.type.stringValue().containsIgnoreCase(keyword)));
    }

    // 페이징 처리
    List<ShippingManager> results = query.offset(pageable.getOffset())
    ...
    List<SippingManagerInfo> infoResults = results.stream()
